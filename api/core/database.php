 ...
 class Database
{
     ...
     public function json_parse(string $json): array
    {
     ...
     {
         if (empty(trim($searchTerm)) || empty($searchableColumns)) {
             return ['status' => 0, 'details' => 'Search term or columns are missing.'];
         }
         ...
         // 1. ساخت عبارات WHERE برای OR/LIKE
         foreach ($searchableColumns as $column) {
             // WHERE (`col1` LIKE ?) OR (`col2` LIKE ?) ...
             $whereClauses[] = "`$column` LIKE ?";
             $values[] = $searchTermWithWildcard;
         }

         // 2. ترکیب شرط‌ها با OR
         $whereString = implode(' OR ', $whereClauses);

         // 3. ساخت کوئری نهایی
         $sql = "SELECT $fieldList FROM `$TblName` WHERE ($whereString)";
         ...
         try {
             ...
             $result = ['status' => 0, 'details' => 'Error searching records: ' . $e->getMessage()];
         }
